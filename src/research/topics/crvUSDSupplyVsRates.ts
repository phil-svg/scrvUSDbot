...
export async function crvUSDSupplyVsRates() {
  ...
  for (let blockNumber = startBlock; blockNumber <= endBlock; blockNumber += blockStep) {
    try {
      ...
      const supply = Number(
        (Number(await web3Call(stablecoinLens, 'circulating_supply', [], blockNumber)) / 1e18).toFixed(0)
      );
      const debt = Number(
        (Number(await web3Call(crvUsdControllerFactory, 'total_debt', [], blockNumber)) / 1e18).toFixed(0)
      );
      const scrvUSD_Supply = Number(
        (Number(await web3Call(scrvUSD, 'totalSupply', [], blockNumber)) / 1e18).toFixed(0)
      );
      ...
      const priceCrvUSD = Number(
        (Number(await web3Call(crvUsdPriceAggregator, 'price', [], blockNumber)) / 1e18).toFixed(5)
      );
